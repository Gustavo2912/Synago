...
function exportCSV(filename: string, rows: any[]) {
  const headers = ["Name", "Email", "Phone", "Position", "Roles", "Organizations", "Status"];
...
export default function Users() {
  ...
  const canEdit = permissions.manage_users || isGlobalSuperAdmin;
  ...
  const filteredUsers = useMemo(() => {
    ...
    } else if (isGlobalSuperAdmin && organizationId && organizationId !== "all") {
      list = list.filter((u) =>
        u.roles.some((r) => r.organization_id === organizationId)
      );
    }
    ...
    if (filters.role) {
      list = list.filter((u) => u.roles.some((r) => r.role === filters.role));
    }
    ...
    list = list.filter((u) => {
      ...
      if (filters.searchName && !fullName.includes(filters.searchName.toLowerCase())) return false;
      if (filters.searchEmail && !email.includes(filters.searchEmail.toLowerCase())) return false;
      if (filters.searchPhone && !phone.includes(filters.searchPhone.toLowerCase())) return false;
      if (filters.searchPosition && !position.includes(filters.searchPosition.toLowerCase()))
        return false;
      if (filters.searchText && !blob.includes(filters.searchText.toLowerCase())) return false;
    ...
    const sorted = [...list].sort((a, b) => {
      ...
      const mapping: Record<string, [any, any]> = {
        ...
        position: [(a.position || "").toLowerCase(), (b.position || "").toLowerCase()],
        status: [a.roles.some((r) => !r.suspended), b.roles.some((r) => !r.suspended)],
      };
    ...
    return sorted;
  }, [teamUsers, filters, sortField, sortDir, organizationId, isGlobalSuperAdmin]);
  ...
  const handleExport = () => {
    const rows = filteredUsers.map((u) => {
      const name = `${u.first_name || ""} ${u.last_name || ""}`.trim() || u.email || u.user_id;
  ...
  const totalRolesAssigned = filteredUsers.reduce((s, u) => s + u.roles.length, 0);

  const rolesBreakdown = useMemo(() => {
    const m = new Map<string, number>();
    filteredUsers.forEach((u) =>
      u.roles.forEach((r) => m.set(r.role, (m.get(r.role) || 0) + 1))
    );
    return [...m.entries()];
  }, [filteredUsers]);
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50/80 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button className="flex gap-2 items-center" onClick={() => setShowAddUser(true)}>
          <UserPlus size={16} />
          {t("users.add")}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-white/80 backdrop-blur border shadow-sm">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-700">{totalRolesAssigned}</div>
          </CardContent>
        </Card>
          ...
          <Table className="w-full text-sm">
            ...
            <TableBody>
              {pageRows.map((u) => {
                ...
                const isActive = u.roles.some((r) => !r.suspended);

                return (
                  <TableRow key={u.user_id}>
                    <TableCell>
                      <span className="font-medium">
                        {highlight(fullName, filters.searchName || filters.searchText)}
                      </span>
                    </TableCell>
                      ...
                      <div className="flex flex-col text-xs text-muted-foreground gap-1">
                        {u.email && (
                          <span className="flex items-center gap-1">
                            ...
                            {highlight(u.email, filters.searchEmail || filters.searchText)}
                          </span>
                        )}
                        {u.phone && (
                          <span className="flex items-center gap-1">
                            <Phone className="w-3 h-3" />
                            {highlight(u.phone, filters.searchPhone || filters.searchText)}
                          </span>
                        )}
                      </div>
                    </TableCell>
