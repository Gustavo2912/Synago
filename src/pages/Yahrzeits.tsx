...
export default function YahrzeitsPage() {
  ...
  /* =======================================================
     ❌ DONATION STATS BY DONOR (NO LONGER USED IN TABLE)
  ======================================================= */
  ...
  /* =======================================================
     ✅ NEW: MAP donation BY yahrzeit_id
  ======================================================= */

  const donationByYahrzeitId = useMemo(() => {
    const map = new Map<string, any>();
    yahrzeitDonations.forEach((d: any) => {
      if (d.yahrzeit_id) {
        map.set(d.yahrzeit_id, d);
      }
    });
    return map;
  }, [yahrzeitDonations]);
                  ...
                  <Table className="text-sm">
                    ...
                    <TableBody>
                      {rows.map((y: any) => {
                        /* =======================================================
                           ❌ OLD:
                           const stats = donationStatsByDonor.get(y.donor_id);

                           ✅ NEW — GET ONLY *THIS Yahrzeit* DONATION:
                        ======================================================= */
                        const donation = donationByYahrzeitId.get(y.id);
