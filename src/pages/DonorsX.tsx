...
export default function Donors() {
  ...
  // ✔ FIX — pass override correctly
  const { data: donationsRaw = [] } = useAllDonations({}, organizationId);

  // ✔ FIX — new signature supports override
  const { data: pledgesRaw = [] } = usePledges(organizationId);

  // no filtering needed — hooks already filter by org
    ...
    <div className="p-6 space-y-6 bg-slate-50/80 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-white/80 backdrop-blur shadow-sm border">
          <CardHeader><CardTitle>{t("donors.totalDonors")}</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="bg-white/80 backdrop-blur shadow-sm border">
          <CardHeader><CardTitle>{t("donors.totalAmountDonated")}</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="bg-white/80 backdrop-blur shadow-sm border">
          <CardHeader><CardTitle>{t("donors.totalAmountPledged")}</CardTitle></CardHeader>
          <CardContent>
              ...
              {pageItems.map((donor: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell onClick={() => openProfile(donor)}>
                      {highlight(phone, filters.searchPhone || filters.searchText)}
                    </TableCell>

                    <TableCell onClick={() => openProfile(donor)}>
                      {highlight(email, filters.searchEmail || filters.searchText)}
                    </TableCell>
