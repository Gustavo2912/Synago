...
export default function DonationsPage() {
  ...
  /* summary */
  const totalAmount = filtered.reduce((sum, d: any) => sum + Number(d.amount), 0);
  const totalDonors = new Set(filtered.map((d: any) => d.donor_id)).size;
            ...
            <tbody>
              {pageItems.map((d: any) => (
                <tr key={d.id} className="border-b hover:bg-gray-50">
                  <td className="p-3">{highlight(d.donor_name, filters.searchText)}</td>
                  <td className="p-3">{highlight(d.donor_phone, filters.searchText)}</td>
                  <td className="p-3">{highlight(d.donor_email, filters.searchText)}</td>
                  ...
                  <td className="p-3 max-w-[200px] truncate">{d.notes}</td>
                </tr>
              ))}

              {pageItems.length === 0 && (
                <tr>
                  <td colSpan={showOrgCol ? 10 : 9} className="p-6 text-center text-gray-500">
