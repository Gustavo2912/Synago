    ...
    <div className="space-y-6 border rounded-xl p-6 bg-card">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          value={props.language ? props.title?.[props.language] ?? "" : ""}
          onChange={(e) => {
            // ✅ FIX: לא כותבים key בשם "undefined"
            if (!props.language) {
              console.error("HomeEditor: missing language; skip title update");
              return;
            }
            props.setTitle(withLang(props.title, props.language, e.target.value));
          }}
        />
      </div>
                  ...
                  <div className="space-y-3">
                    {col.blocks.map((block) => (
                      <div key={block.id} className="border rounded-lg p-3 relative space-y-3">
                        ...
                        {block.type === "text" && (() => {
                          const b: any = block;
                          const textValue = b.content?.[props.language] ?? "";
                          ...
                          return (
                            <>
                              ...
                              <Input
                                ...
                                value={b.caption?.[props.language] ?? ""}
                                onChange={(e) =>
                                  updateBlock(row.id, col.id, block.id, {
