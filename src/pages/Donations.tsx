...
export default function DonationsPage() {
  ...
  const filtered = useMemo(() => {
    return donationsRaw.filter((d: any) => {
      ...
      if (filters.searchName) {
        if (!d.donor_name?.toLowerCase().includes(filters.searchName.toLowerCase()))
          return false;
      }
      ...
      if (filters.searchEmail) {
        if (!d.donor_email?.toLowerCase().includes(filters.searchEmail.toLowerCase()))
          return false;
      }
  ...
  const totalAmountByCurrency = useMemo(() => {
    ...
    (filtered as any[]).forEach((d) => {
      ...
      const cur = (orgId && orgCurrencyMap.get(orgId)) || fallbackCurrency;
    ...
    <div className="p-6 space-y-6 bg-slate-50/80 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border bg-white/80 shadow-sm backdrop-blur">
          <CardHeader><CardTitle>{t("donations.totalAmount")}</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border bg-white/80 shadow-sm backdrop-blur">
          <CardHeader><CardTitle>{t("donations.totalDonors")}</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border bg-white/80 shadow-sm backdrop-blur">
          <CardHeader><CardTitle>{t("donations.totalCount")}</CardTitle></CardHeader>
          <CardContent>
        ...
        <div className="max-h-[70vh] overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50/80 backdrop-blur sticky top-0 z-10">
              <tr>
                {[
                  ...
                  ...(showOrgCol ? [["organization_name", "Organization"]] : []),
            ...
            <tbody>
              {pageItems.map((d: any) => {
                const orgId = d.organization_id;
                const cur = orgCurrencyMap.get(orgId) || fallbackCurrency;
                ...
                return (
                  <tr key={d.id} className="border-b hover:bg-gray-50">
                    ...
                    {/* NAME */}
                    <td className="p-3">
                      {highlight(d.donor_name, filters.searchName || filters.searchText)}
                    </td>

                    {/* PHONE */}
                    <td className="p-3">
                      {highlight(d.donor_phone, filters.searchPhone || filters.searchText)}
                    </td>

                    {/* EMAIL */}
                    <td className="p-3">
                      {highlight(d.donor_email, filters.searchEmail || filters.searchText)}
                    </td>
                    ...
                    {/* TYPE */}
                    <td className="p-3">
                      {highlight(d.type, filters.searchText)}
                    </td>

                    {/* METHOD */}
                    <td className="p-3">
                      {highlight(d.payment_method, filters.searchText)}
                    </td>

                    {/* CAMPAIGN */}
                    <td className="p-3">
                      {highlight(d.campaign_name, filters.searchText)}
                    </td>

                    {/* NOTES */}
                    <td className="p-3 max-w-[200px] truncate">
                      {highlight(d.notes, filters.searchText)}
                    </td>
                  </tr>
                );
              })}
