...
export default function PaymentsPage() {
  const { t, currency: fallbackCurrency } = useLanguage();
  ...
  /* -----------------------------------------------------------
     LOAD SETTINGS FOR ALL ORGS (CURRENCIES)
----------------------------------------------------------- */
...
----------------------------------------------------------- */
  const filtered = useMemo(() => {
    return payments.filter((p: any) => {
      ...
      if (filters.searchText && !haystack.includes(filters.searchText.toLowerCase()))
        return false;
...
----------------------------------------------------------- */
  const handleExport = () => {
    const rows = (sorted as any[]).map((p) => {
      const orgId = p.organization_id;
      const cur =
        (orgId && orgCurrencyMap.get(orgId)) || fallbackCurrency;
      ...
      return {
        ...
        amount: `${cur} ${Number(p.amount || 0)}`,
        method: p.method,
        date: p.date || p.created_at,
        notes: p.notes,
        pledge_total: p.pledge ? `${cur} ${p.pledge.total_amount}` : "",
        pledge_balance: p.pledge ? `${cur} ${p.pledge.balance_owed}` : "",
        pledge_due: p.pledge ? p.pledge.due_date : "",
      };
    });
    ...
    <div className="p-6 space-y-6 bg-slate-50/80 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-white/80 backdrop-blur shadow">
          ...
          <CardContent className="text-3xl font-bold">{totalPayments}</CardContent>
        </Card>
      </div>
          ...
          <Table className="w-full text-sm">
            ...
            <TableBody>
              {pageItems.map((p) => {
                ...
                const orgCurrency =
                  (p.organization_id && orgCurrencyMap.get(p.organization_id)) ||
                  fallbackCurrency;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="p-3">
                      {highlight(p.donor_name, filters.searchName || filters.searchText)}
                    </TableCell>

                    {/* PHONE */}
                    <TableCell className="p-3">
                      {highlight(p.donor_phone, filters.searchPhone || filters.searchText)}
                    </TableCell>

                    {/* EMAIL */}
                    <TableCell className="p-3">
                      {highlight(p.donor_email, filters.searchEmail || filters.searchText)}
                    </TableCell>

                    {/* ORG NAME */}
                    {showOrgColumn && (
                      <TableCell className="p-3">
                        {highlight(p.organization_name, filters.searchText)}
                      </TableCell>
                    )}

                    {/* AMOUNT */}
                    <TableCell className="p-3">
                      {orgCurrency} {Number(p.amount).toLocaleString()}
                    </TableCell>

                    {/* DATE */}
                    <TableCell className="p-3">{fmtDate(paymentDate)}</TableCell>
                    ...
                    <TableCell className="p-3 text-xs leading-tight">
                      {p.pledge ? (
                        <>
                          <div>
                            {t("payments.pledgeTotal")}: {orgCurrency}{" "}
                            {Number(p.pledge.total_amount).toLocaleString()}
                          </div>
                          <div>
                            {t("payments.pledgeBalance")}: {orgCurrency}{" "}
                            {Number(p.pledge.balance_owed).toLocaleString()}
                          </div>
                          <div>
                            {t("payments.dueDate")}: {fmtDate(p.pledge.due_date)}
                          </div>
                        </>
                      ) : (
                        <span className="text-gray-400">{t("payments.none")}</span>
                      )}
                    </TableCell>
