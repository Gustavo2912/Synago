...
export function AppSidebar() {
  ...
  const { t, language } = useLanguage();   // ← הוספנו language
  const { permissions, userLoading, user, setOrganizationId } = useUser();
  const navigate = useNavigate();

  if (userLoading || !user) return null;

  /** RTL: אם השפה היא עברית → הסיידבר בצד ימין */
  const side = language === "he" ? "right" : "left";

  /** ------------------------
   *  FIXED: Full Session Logout
   * ------------------------ */
  ...
  return (
    <Sidebar
      ...
      className={cn(
        ...
        "border-border/50",
        language === "he" && "rtl"   // ← הוספנו תמיכה ב־RTL
      )}
    >
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          cn(
                            ...
                            language === "he" && "flex-row-reverse text-right"  // ← תמיכה ב־RTL
                          )
                        }
                      >
