...
export default function EditYahrzeitDialog({ open, onClose, yahrzeit }: any) {
  ...
  const [donationCurrency, setDonationCurrency] = useState(defaultCurrency);

  const mustChooseOrg =
    isGlobalSuperAdmin && (organizationId === "all" || !organizationId);
  ...
  useEffect(() => {
    ...
    const orgId =
      mustChooseOrg ? yahrzeit.organization_id : organizationId || yahrzeit.organization_id;
    ...
    setSameAsDonor(yahrzeit.contact_email === yahrzeit.donor_email);
  ...
  }, [open, yahrzeit, organizationId]);

  /* ------------------------------------------
     AUTO-FILL CONTACT WHEN SAME AS DONOR
  ------------------------------------------ */
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    const finalOrg =
      selectedOrg ||
      (!mustChooseOrg ? organizationId : null) ||
      yahrzeit.organization_id;

    const amountNum = Number(donationAmount);
    if (isNaN(amountNum) || amountNum < 0)
      return toast.error("Invalid donation amount");
    ...
    try {
      ...
      /* Currency is always org default — NOT editable */
      await upsertDonation.mutateAsync({
        ...
        amount: amountNum,
        currency: donationCurrency, // locked to defaultCurrency
      });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, secular_date: e.target.value })
                  }
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, hebrew_date: e.target.value })
                  }
                />
              </div>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, prayer_text: e.target.value })
                }
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(v) =>
                  setForm({ ...form, reminder_enabled: Boolean(v) })
                }
              />
          ...
          {/* DONATION — Currency is READ-ONLY */}
