// src/components/AddDonorDialog.tsx
...
export default function AddDonorDialog({ open, onClose }: any) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setSelectedOrgs(
        isGlobalSuperAdmin ? [] : [organizationId] // user רגיל — רק הארגון שלו
      );
    }
  }, [open]);
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3 py-2">
          ...
          {/* הסופר אדמין בלבד רואה את בחירת הארגונים */}
          {isGlobalSuperAdmin && (
            <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelectedOrgs(selectedOrgs.filter((x) => x !== o.id));
                      }
                    }}
                  />
                  {o.name}
